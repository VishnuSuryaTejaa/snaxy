    ...
    >
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-900/60">
        ...
        {item.isSoldOut && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-20">
            <span className="text-xs font-black tracking-widest text-rose-400 uppercase border border-rose-500/40 rounded-full px-4 py-1.5 bg-rose-950/60">
              Sold Out
            </span>
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 z-10">
          <span className="text-[11px] font-extrabold px-2.5 py-1 rounded-xl bg-black/60 backdrop-blur-md text-slate-200 border border-white/10 shadow-sm">
            {item.category}
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-5 gap-3">
        <div className="flex-1">
          <h3 className="font-extrabold text-base sm:text-lg leading-tight tracking-tight text-white group-hover:text-orange-400 transition-colors font-heading">
            {item.name}
          </h3>
          {item.description && (
        ...
        <div className="flex items-center justify-between gap-3 mt-auto pt-2 border-t border-white/[0.06]">
          ...
          {item.isSoldOut ? null : quantity === 0 ? (
            <button
              onClick={handleAdd}
              className="flex items-center gap-1.5 rounded-2xl bg-gradient-to-r from-primary to-rose-500 px-4 py-2.5 text-xs font-black text-white hover:scale-105 hover:shadow-[0_0_20px_rgba(255,94,14,0.5)] transition-all duration-200 active:scale-95 shadow-md border border-white/15"
            >
